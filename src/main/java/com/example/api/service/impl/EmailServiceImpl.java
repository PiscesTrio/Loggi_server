 ...
 *
 * <p>What this replaces was not an unfinished feature, it was an unlocked door. The code
 * was the literal {@code "123456"}, {@code mailSender.send(...)} sat commented out, and the
 * endpoint that issues codes is public — so two unauthenticated requests produced a
 * {@code ROLE_SUPER_ADMIN} token for any address the caller could name. Confirmed against a
 * running server before this rewrite, not inferred from the source.
 *
 ...
 *   <li>it is stored hashed and expiring, so reading the store hands out nothing live and
 *       nothing accumulates;
 *   <li>guessing and sending are both rate-limited, so six digits is a real space.
 * </ul>
 *
 * <p>Redis rather than the JPA {@code Code} table this used to write. A one-time code is
 * cache data: short-lived, single-use, write-heavy. {@code SET ... EX} expires it without
 * the application-level {@code exp} column that nothing ever cleaned up, and {@code INCR}
 * makes the attempt counter atomic — a read-then-write over JPA loses count under
 * concurrency, and concurrency is the one condition an attacker controls freely.
 */
@Slf4j
@Service
public class EmailServiceImpl implements EmailService {
    ...
    public void sendVerificationCode(String email) {
        ...
        Boolean claimed = redis.opsForValue().setIfAbsent(
                COOLDOWN_KEY + email, "1", Duration.ofSeconds(properties.sendCooldownSeconds()));
     ...
     *
     * <p>INCR is atomic, so two parallel guesses cannot both read 4 and both write 5. The
     * TTL is set on the first increment only — refreshing it on every attempt would let a
     * patient attacker hold the window open indefinitely without ever reaching the limit.
     */
    private void countFailure(String email) {
        ...
        if (attempts >= properties.maxAttempts()) {
            redis.opsForValue().set(
                    LOCK_KEY + email, "1", Duration.ofSeconds(properties.lockSeconds()));
            redis.delete(ATTEMPTS_KEY + email);
            redis.delete(CODE_KEY + email);
