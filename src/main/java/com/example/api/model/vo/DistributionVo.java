 ...
 *
 * <p>This replaces serialising the entity, and it settles something S09 left awkward. Once
 * the order pointed at real rows, returning the entity meant returning whole Driver, Vehicle
 * and Warehouse objects nested inside every order - each with its own timestamps, its own
 * id-card number, its own everything - because Jackson serialises what the object graph
 * holds, not what a caller needs. The alternative available at the time was to emit a bare
 * id, which took the driver's name away from a screen that displays it.
 *
 * <p>A view type does not have to choose. Each reference becomes a summary carrying the two
 * or three fields a client actually renders, and the fields it does not need never leave the
 * server. The old response had both a {@code did} and a {@code driver} holding a copied name;
 * this has one driver, and the name in it is the driver's current name rather than whatever
 * was copied when the order was filed.
 */
public record DistributionVo(
     ...
     *
     * <p>{@code DistributionRepository.findAll} declares an entity graph for exactly this
     * reason. Mapping outside a transaction without one throws LazyInitializationException -
     * which is the same failure that used to happen during serialisation, moved somewhere a
