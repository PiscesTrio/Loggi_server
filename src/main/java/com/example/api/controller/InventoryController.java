...
public class InventoryController {
    ...
    public List<InventoryRecordVo> findRecordByWarehouse(@PathVariable String id) {
        return recordService.findAllByWarehouseId(id).stream().map(InventoryRecordVo::from).toList();
    }
    ...
    public List<InventoryRecordVo> findRecordByCommodity(@PathVariable String id) {
        return recordService.findAllByCommodityId(id).stream().map(InventoryRecordVo::from).toList();
    }
     ...
     *
     * <p>References become entities holding only an id, which is what
     * InventoryRecordServiceImpl resolves before writing - so a warehouse or commodity that
     * does not exist is a 404 naming it, not a foreign-key violation answered with "this
     * already exists".
     */
