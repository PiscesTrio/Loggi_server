 ...
 *
 * <p>Extends {@link OncePerRequestFilter} rather than {@code BasicAuthenticationFilter}:
 * the latter means "parse HTTP Basic credentials" and was only being borrowed for its
 * filter plumbing, at the price of having to feed it an {@code AuthenticationManager}
 * that was never used.
 *
 * <p>Three outcomes, and only three:
 * <ul>
 *   <li>no {@code Bearer} token — continue unauthenticated, and let the authorization
 *       layer decide whether that is acceptable for the endpoint;</li>
 *   <li>a token that verifies — populate the security context and continue;</li>
 *   <li>a token that does not verify, for any reason — <strong>401</strong>.</li>
 * </ul>
 *
 * <p>That last case is the behaviour change. Previously a forged or malformed token
 * escaped as an uncaught exception (HTTP 500, leaking a stack trace) while an expired
 * one produced a 200 whose body claimed 403 — neither of which a client can act on.
 */
