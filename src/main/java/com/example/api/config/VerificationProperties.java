 ...
 *
 * <p>These are the numbers that decide whether a six-digit code is a credential or a formality, so
 * they are configuration rather than literals buried in a service: a reviewer can see all four at
 * once, and a deployment can tighten them without a rebuild.
 *
...
public record VerificationProperties(
        long codeTtlSeconds, long sendCooldownSeconds, int maxAttempts, long lockSeconds) {}
