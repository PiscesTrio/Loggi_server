 ...
 *
 * <p>springdoc derives a response schema from the controller's return type. This API does not send
 * that: {@code GlobalResponseHandler} is a {@code ResponseBodyAdvice} that wraps every success into
 * {@code {code, status, msg, data}} <em>after</em> the method returns, so the document claimed
 * {@code GET /api/commodity} answers with an array of CommodityVo while the server answers with an
 * envelope containing one.
 *
 * <p>That gap is not cosmetic. It is the reason a client generated from this document would fail to
 * parse every single response - it would look for the array where the envelope is - and it makes
 * "OpenAPI as the single source of truth" untrue while it stands.
 *
 * <p>So the two follow one rule. This customizer applies the same decision the advice makes: wrap
 * unless the handler opted out with {@link DisableBaseResponse}, and never wrap a 204, which
...
public class EnvelopeOpenApiCustomizer {
     ...
     *
     * <p>An {@link OperationCustomizer} is the only hook that sees the handler method, and the
     * annotation is the only way to know an endpoint bypasses the advice. Nothing in production
     * uses it today; it is honoured anyway, because the alternative is a document that silently
     * becomes wrong the first time somebody does.
     */
    @Bean
     ...
     *
     * <p>Named rather than inline: a generator turns an inline object into an anonymous model with
     * a name derived from its position, so the client ends up with types called things like {@code
     * InlineResponse200}. {@code ResponseResultCommodityVoList} says what it is.
     */
    @Bean
                    ...
                    .forEach(
                            pathItem ->
                                    pathItem.readOperations()
                                            .forEach(operation -> wrap(operation, components)));
        };
    }

    private void wrap(Operation operation, Components components) {
        if (operation.getExtensions() != null
                && operation.getExtensions().remove(OPT_OUT) != null) {
            return;
        }
        ...
        operation
                ...
                .forEach(
                        (status, response) -> {
                            // 204 is the one response with no envelope, because a body
                            // contradicting a
     ...
     *
     * <p>Returns null for anything without a referenced type - a bare string or number - and the
     * caller inlines those. Naming them would produce {@code ResponseResultString}, which reads
     * like a type this codebase has and does not.
     */
    private String wrapperNameFor(Schema<?> data) {
        ...
        if (data instanceof ArraySchema array
                && array.getItems() != null
    ...
    private Schema<Object> envelopeSchema(Schema<?> data) {
        ...
        envelope.addProperty(
                "msg", new StringSchema().description("Set on a failure.").nullable(true));
